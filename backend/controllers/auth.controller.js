...
async function registrationController(req, res) {
  try {
    ...
    if (existingMember) {
      return res
        .status(400)
        .json({ error: "Username already exist. Please choose another one." });
    }
  ...
  try {
    ...
    if (!memberCreds) {
      return res.status(400).json({
        error:
          "There's no account with this username. Please create an account.",
      });
    }
    ...
    if (isPwdValid) {
      return res.status(201).json({
        success: "Credentials valid. Access authorized.",
        username: memberCreds.username,
      });
    } else {
      res.status(400).json({
        error: "Username and password not correct. Please try again.",
      });
    }
  } catch (error) {
...
async function logoutController(req, res) {
  req.session.destroy((error) => {
    if (error) {
      return res
        .status(500)
        .json({ error: "Error while trying to log out. Please try again." });
    }
